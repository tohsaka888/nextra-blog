...
另一种方法是，创建一个数组，它包含`1000`个指向char的指针，然后使用`new`根据每个字符串的需要分配相应数量的内存。
...
获得空间后，`getname()`使用标准库函数`strcpy()`将temp中的字符串复制到新的内存块中。
...
当程序控制权回到`main()`时，`temp`使用的内存将自动被释放。如果`getname()`返回temp的地址，
...
它们管理了一个内存池，这在C++中被称为**自由存储空间**（_free store_）或**堆**（_heap_）。
